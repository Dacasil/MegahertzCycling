...
def vibration_intensity(cdw_spots, cdw_rad, drift_file_path):
    ...
    for i in tqdm(range(len(drift_data))):
        ...
        intensity_main = func.intensity_in_image(cdw_spots, cdw_rad, shape_of_data, file) # Use the intensity function
        normed_intensity = (intensity_main/all_intensity) *100
...
path_of_data_raw = r'Data\Processed_Data\A7_processed\Integrated_images\Rreflectraw' # Enter path of the folder of which one wants to get the spot coordinates
drift_corrected_file_path = r'Data\Processed_Data\A7_processed\Drift_corrected_images\Rreflect' # File path where the drift corrected images are stored
integrated_data = r'Data\Processed_Data\A7_processed\Integrated_images\Rreflectraw' # Must have the same order as the drift corrected images
integrated_data_files = os.listdir(integrated_data)
zero_data_name = "0kHz_0mW_drift_corrected_summed_intensity.tif" # 0mW/0kHz integrated data folder name
...
folder = "4_10mW"
name_of_sample = "A7"
reprate_fluence_name = "Rreflect_cdw"
numbers = 4  # number to get the intgrated data fittning to the drift corrected files
...
for l in range(0,len(cdw_spots_1)):
    plt.scatter(cdw_spots_1[l][0], cdw_spots_1[l][1], facecolors='none', edgecolors='b', s=60)
plt.imshow(FilteredImage,cmap='gray')
plt.show()

for l in range(0,len(cdw_spots_2)):
    plt.scatter(cdw_spots_2[l][0], cdw_spots_2[l][1], facecolors='none', edgecolors='r', s=60)
plt.imshow(FilteredImage,cmap='gray')
plt.show()

for l in range(0,len(cdw_spots_3)):
    plt.scatter(cdw_spots_3[l][0], cdw_spots_3[l][1], facecolors='none', edgecolors='y', s=60)
plt.imshow(FilteredImage,cmap='gray')
plt.show()
...
vibration_1 = vibration_intensity(cdw_spots_1,radius_cdw, drift_file_path)
print(len(vibration_1))
vibration.append(vibration_1)
vibration_2 = vibration_intensity(cdw_spots_2, radius_cdw, drift_file_path)
vibration.append(vibration_2)
vibration_3 = vibration_intensity(cdw_spots_3, radius_cdw, drift_file_path)
